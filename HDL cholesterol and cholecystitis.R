...
r_squared<-(2*(1-eaf.exposure)*eaf.exposure*beta.exposure)/(se.exposure*187167^0.5)
HDL<-cbind(HDL,fstatistic.exposure,r_squared)
sum(r_squared,na.rm = TRUE)
detach(HDL)
dim(HDL)
#write.csv(HDL,file = "D:/mr/t2d and cholecystitis/新建文件夹/HDL exopusre.csv")
...
#mydata<-subset(mydata, SNP!='rs102275' )
sum(mydata$r_squared,na.rm = TRUE)
write.csv(mydata[,c('SNP','effect_allele.exposure','other_allele.exposure','effect_allele.outcome','other_allele.outcome','beta.exposure','beta.outcome','se.outcome',"se.exposure",'pval.outcome','pval.exposure','fstatistic.exposure')],file = "D:/mr/t2d and cholecystitis/新建文件夹/HDL identified SNPs.csv")
...
write.csv(newresult[,c('method','pval','OR','upper','lower')],
          file = "D:/mr/t2d and cholecystitis/新建文件夹/result of HDL.csv")
...
write.csv(het[,c('method','Q_pval','Q')],
          file = "D:/mr/t2d and cholecystitis/新建文件夹/heterogeneity test of HDL.csv")

pleio<-mr_pleiotropy_test(mydata)
write.csv(pleio[,c('egger_intercept','pval')],
          file = "D:/mr/t2d and cholecystitis/新建文件夹/pleiotropy test of HDL.csv")
